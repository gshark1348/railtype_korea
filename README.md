...
KTX 호남고속선 화면에서 역이 국토 경계와 어긋나 보이던 문제를 수정했습니다. 조사 결과 역 좌표보다 **대한민국 전국 경계 링의 연결 순서 오류**가 더 큰 원인이었으며, 서북부 일부 점이 멀리 떨어진 점과 직선으로 이어지면서 전국 지도 자체가 왜곡되고 있었습니다.

- 대한민국 본토 국경선과 해안선을 연속된 순서로 다시 구성
- 대각선으로 국토를 가로지르던 비정상 경계 구간 제거
- 본토·제주·거제·진도·남해·강화의 유효한 `MultiPolygon` 생성
- 용산–목포 10개 역의 WGS84 좌표 재검수 및 정규화
- 기존 v26.0 대비 역사별 좌표 조정 범위: 약 0–85.5 m
- 보정 후 10개 역사 모두 대한민국 경계 내부 위치 확인
- 전국 지도 자동 투영 범위를 본토와 제주 기준으로 안정화
...
### v26.1 KTX 호남고속축 위치 보정

v26.0의 전국 경계 데이터에서 서로 인접하지 않은 점이 연결되며 생긴 비정상 대각선을 제거하고, 본토 국경선과 해안선을 연속된 순서로 재구성했습니다. 그 위에 재검수한 용산–목포 10개 역 좌표를 중첩해 육지 포함 여부와 경계 위상 유효성을 검사했습니다.

![KTX 호남고속축 위치 보정 플롯](docs/ktx-honam-position-audit-v26.1.png)

상세 결과는 [`VALIDATION-v26.1.md`](VALIDATION-v26.1.md)와 [`data/ktx-honam-validation-v26.1.json`](data/ktx-honam-validation-v26.1.json)에서 확인할 수 있습니다. 역 사이의 화면 선은 실제 철도 측량 선형이 아니라 역 좌표를 연결한 인포그래픽 중심선입니다.
...
| **v26.1** | 전국 경계 링 연결 순서 오류 수정, KTX 호남 10개 역사 좌표 재검수, 자동 투영 범위와 GIS 검증 자료 갱신 |
...
- [`VALIDATION-v26.1.md`](VALIDATION-v26.1.md)
- [`data/ktx-honam-validation-v26.1.json`](data/ktx-honam-validation-v26.1.json)
...
│   ├── ktx-honam-audit-v26.png       # 호남고속축 최초 GIS 검증
│   └── ktx-honam-position-audit-v26.1.png # 호남고속축 위치 보정
├── js/
...
│   ├── national-boundary.js          # 위상 보정된 대한민국 전국 경계
...
└── data/
    ...
    ├── ktx-honam-validation-v26.1.json # 호남 위치 보정 검증
...
git init
git add .
git commit -m "Release RAILTYPE KOREA v26.1"
git branch -M main
